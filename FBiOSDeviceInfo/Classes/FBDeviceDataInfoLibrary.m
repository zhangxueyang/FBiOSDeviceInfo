...
@interface FBDeviceDataInfoLibrary ()
@property (nonatomic, assign) DiviceType iDevice;

@end


static FBDeviceDataInfoLibrary *deviceDataInfo = nil;

@implementation FBDeviceDataInfoLibrary

+(instancetype)sharedDeviceDataInfoLibrary {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        deviceDataInfo = [[super allocWithZone:NULL] init];
        deviceDataInfo.iDevice = [self transformMachineToIdevice];
    });
    return deviceDataInfo;
}
+(id)allocWithZone:(struct _NSZone *)zone{
    return [FBDeviceDataInfoLibrary sharedDeviceDataInfoLibrary];
}
